...
/**
 * Fetch a Payment Entry PDF from Frappe's print API.
 * Returns the PDF buffer, or null if fetch fails.
 */
export async function fetchPaymentEntryPDF(paymentEntryId: string): Promise<Buffer | null> {
  const FRAPPE_URL = process.env.NEXT_PUBLIC_FRAPPE_URL;
  const API_KEY = process.env.FRAPPE_API_KEY;
  const API_SECRET = process.env.FRAPPE_API_SECRET;
  ...
  try {
    const url = `${FRAPPE_URL}/api/method/frappe.utils.print_format.download_pdf?doctype=Payment+Entry&name=${encodeURIComponent(paymentEntryId)}&no_letterhead=1`;
    const res = await fetch(url, {
      headers: { Authorization: `token ${API_KEY}:${API_SECRET}` },
      cache: "no-store",
    });
    ...
    if (!res.ok) {
      console.warn(`[email] PDF fetch failed for ${paymentEntryId}: ${res.status}`);
      return null;
    }
    ...
    console.warn("[email] Payment Entry PDF fetch error:", err);
    return null;
  }
}
