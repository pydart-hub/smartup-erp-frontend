...
import { getGMMentorStudentDetail } from "@/lib/api/mentors";
...
export default function GMMentorStudentDetailPage() {
  const { id } = useParams<{ id: string }>();
  const studentId = decodeURIComponent(id);
  const [attendanceMode, setAttendanceMode] = useState<"summary" | "absent">("summary");

  const { data, isLoading, error } = useQuery({
    queryKey: ["gm-mentor-student-detail", studentId],
    queryFn: () => getGMMentorStudentDetail(studentId),
    staleTime: 30_000,
  });
        ...
        <CardContent className="relative p-0">
          ...
          <div className="relative grid gap-6 p-6 lg:grid-cols-[1.4fr_0.9fr] lg:p-8">
            <div className="space-y-5">
              <div>
                <Link href="/dashboard/general-manager/mentor-feedback" className="inline-flex items-center gap-1.5 text-xs font-semibold text-primary hover:underline mb-3">
                  <ArrowLeft className="h-3.5 w-3.5" /> Back to General Manager Mentor Feedback
                </Link>
                <div className="flex items-start gap-4 flex-wrap">
                  ...
                  <div className="min-w-0 flex-1">
                    ...
                    <p className="mt-3 max-w-2xl text-sm leading-6 text-slate-600">
                      General Manager view of the complete mentor-linked student profile with academic, attendance, fee, parent, and feedback visibility.
                    </p>
                  </div>
                </div>
              </div>
            ...
            <div className="rounded-[28px] border border-white/70 bg-white/70 p-5 shadow-[inset_0_1px_0_rgba(255,255,255,0.9),0_18px_40px_-26px_rgba(13,61,89,0.38)] backdrop-blur-xl">
              ...
              <div className="mt-4 space-y-3">
                <div className="rounded-2xl border border-slate-200/80 bg-[linear-gradient(180deg,rgba(248,252,255,0.9),rgba(241,247,250,0.95))] p-4 shadow-[inset_0_1px_0_rgba(255,255,255,0.8)]">
                  <p className="text-xs font-semibold uppercase tracking-[0.18em] text-slate-400">Student Overview</p>
        ...
        <PanelCard
          ...
          action={
            <div className="flex items-center gap-2 rounded-full border border-slate-200 bg-white/80 p-1 shadow-sm">
              <button onClick={() => setAttendanceMode("summary")} className={`rounded-full px-3 py-1.5 text-xs font-semibold transition-all ${attendanceMode === "summary" ? "bg-primary text-white shadow-sm" : "text-slate-500 hover:text-slate-800"}`}>Summary</button>
              <button onClick={() => setAttendanceMode("absent")} className={`rounded-full px-3 py-1.5 text-xs font-semibold transition-all ${attendanceMode === "absent" ? "bg-rose-500 text-white shadow-sm" : "text-slate-500 hover:text-slate-800"}`}>Absent Days</button>
            </div>
          }
        >
          <div className="grid grid-cols-2 md:grid-cols-4 gap-3">
            ...
            <button type="button" onClick={() => setAttendanceMode("absent")} className="text-left">
                ...
                <div className="space-y-2 max-h-72 overflow-y-auto pr-1">
                  {(data.attendance || []).length === 0 ? (
                    ...
                    data.attendance!.slice(0, 12).map((row) => (
                      <div key={row.name} className="flex items-center justify-between rounded-[22px] border border-slate-200/80 bg-[linear-gradient(180deg,rgba(255,255,255,0.98),rgba(245,248,250,0.95))] px-4 py-3 shadow-[0_14px_28px_-24px_rgba(15,23,42,0.55)]">
                        <div>
                        ...
                        <Badge variant={row.status === "Present" ? "success" : row.status === "Late" ? "warning" : "outline"}>{row.status}</Badge>
                      </div>
                    ))
                  )}
                </div>
                ...
                {attendanceStats.absentRows.length === 0 ? (
                  ...
                  <div className="space-y-2 max-h-72 overflow-y-auto pr-1">
                    {attendanceStats.absentRows.map((row) => (
                      <div key={row.name} className="flex items-center justify-between rounded-[22px] border border-rose-200/80 bg-[linear-gradient(180deg,rgba(255,250,250,0.96),rgba(255,240,240,0.92))] px-4 py-3 shadow-[0_14px_28px_-24px_rgba(225,29,72,0.55)]">
                        <div>
                          <p className="font-semibold text-slate-900">{row.date}</p>
                          <p className="text-xs text-slate-500">{row.student_group || "Student group not set"}</p>
                        </div>
        ...
        {!data.feedback || data.feedback.length === 0 ? (
          ...
          <div className="space-y-4">
            {data.feedback.map((row) => (
              <div key={row.name} className="rounded-[24px] border border-slate-200/80 bg-[linear-gradient(180deg,rgba(255,255,255,0.98),rgba(243,248,250,0.95))] p-5 shadow-[0_18px_30px_-24px_rgba(15,23,42,0.48)]">
                <div className="flex items-start justify-between gap-3 flex-wrap">
                  <div>
                    ...
                    <div className="mt-3 flex items-center gap-3 flex-wrap text-xs text-slate-500">
                      <span className="inline-flex items-center gap-1"><PhoneCall className="h-3.5 w-3.5" />{row.contact_person || "Parent"}{row.contact_number ? ` • ${row.contact_number}` : ""}</span>
                ...
                {(row.overall_feedback || row.academic_notes || row.fee_notes || row.contact_notes) ? (
                  <div className="mt-4 grid gap-3 md:grid-cols-2">
                    {row.academic_notes ? <FeedbackNote title="Academic Notes" body={row.academic_notes} /> : null}
                    {row.fee_notes ? <FeedbackNote title="Fee Notes" body={row.fee_notes} /> : null}
                    {row.contact_notes ? <FeedbackNote title="Contact Notes" body={row.contact_notes} /> : null}
                    {row.overall_feedback ? <div className="md:col-span-2"><FeedbackNote title="Overall Feedback" body={row.overall_feedback} /></div> : null}
                  </div>
                ) : null}
              </div>
            ))}
...
function HeroMetric({ label, value, icon, tone = "default" }: { label: string; value: string; icon: React.ReactNode; tone?: "default" | "mint" | "amber"; }) {
  const tones = {
...
function MiniRow({ label, value }: { label: string; value: string }) {
  return <div className="flex items-center justify-between gap-3"><span className="text-slate-500">{label}</span><span className="font-semibold text-slate-900 text-right">{value}</span></div>;
}

function StatPill({ label, value, tone = "default", interactive = false }: { label: string; value: string; tone?: "default" | "success" | "warning" | "danger"; interactive?: boolean; }) {
  const tones = {
  ...
  return <div className={`rounded-[22px] border px-4 py-3 shadow-[inset_0_1px_0_rgba(255,255,255,0.9),0_14px_24px_-22px_rgba(15,23,42,0.45)] transition-transform ${interactive ? "hover:-translate-y-0.5 cursor-pointer" : ""} ${tones[tone]}`}><p className="text-[10px] font-semibold uppercase tracking-[0.18em]">{label}</p><p className="mt-2 text-2xl font-bold">{value}</p></div>;
}

function FeedbackNote({ title, body }: { title: string; body: string }) {
  return <div className="rounded-[20px] border border-slate-200/80 bg-white/80 p-4 shadow-[inset_0_1px_0_rgba(255,255,255,0.92)]"><p className="text-[11px] font-semibold uppercase tracking-[0.16em] text-slate-400">{title}</p><p className="mt-2 whitespace-pre-wrap text-sm leading-6 text-slate-700">{body}</p></div>;
}
