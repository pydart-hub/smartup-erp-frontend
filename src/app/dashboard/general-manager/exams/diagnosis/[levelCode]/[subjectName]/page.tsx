      ...
      <div className="flex items-center justify-between gap-3 flex-wrap">
        <div>
          <div className="flex items-center gap-2">
            <Link href={`/dashboard/general-manager/exams/diagnosis/${levelCode}`}>
      ...
      <section className="space-y-4">
        ...
        <div className="grid gap-4 md:grid-cols-2 xl:grid-cols-3">
          {data.branchSummaries.map((branch) => (
            <Link
              key={branch.branch}
              href={`/dashboard/general-manager/exams/diagnosis/${levelCode}/${encodeURIComponent(decodedSubject)}/${encodeURIComponent(branch.branch)}`}
              className="block"
            >
