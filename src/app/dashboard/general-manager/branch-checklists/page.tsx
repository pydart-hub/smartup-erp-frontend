...
export default function GeneralManagerBranchChecklistsPage() {
  ...
  const { data: checklists = [], isLoading } = useQuery({
    queryKey: ["branch-checklists-gm", filterBranch, filterDate, filterStatus],
    queryFn: () =>
      getBranchChecklists({
      ...
      <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
        ...
        <div>
          <Badge variant="outline" className="px-3 py-1 bg-white/50 backdrop-blur-sm flex items-center gap-1.5 border-primary/10">
            ...
            <span>Role: <strong>General Manager</strong></span>
          </Badge>
        </div>
      </div>
                            ...
                            <div className="space-y-4">
                              <div>
                                <label className="block text-xs font-semibold text-text-secondary uppercase tracking-wider mb-2">
                                  General Manager Verification Feedback / Remarks
                                </label>
                                <textarea
