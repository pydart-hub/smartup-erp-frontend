...
export default function InstructorCourseSchedulePage() {
  ...
  const calDates = useMemo(
    () => getMonthDates(calYear, calMonth),
    [calYear, calMonth]
  );
  const calFrom = toISODate(calDates[0]);
  const calTo = toISODate(calDates[calDates.length - 1]);

  // Branch filter options – only the instructor's allowed companies
  const branchOptions = useMemo(() => {
    return [...allowedCompanies].sort();
  }, [allowedCompanies]);
  ...
  // Always restrict to instructor's allowed branches.
  // When a specific branch is chosen in the dropdown, narrow further.
  const { data: schedRes, isLoading } = useQuery({
    ...
    queryFn: () =>
      getCourseSchedules({
        from_date: fromDate,
        to_date: toDate,
        // single branch selected → use exact filter; otherwise restrict to allowed set
        branch: branchFilter || undefined,
        branches: !branchFilter && allowedCompanies.length > 0
          ? allowedCompanies
          : undefined,
    ...
    if (search) {
      ...
      list = list.filter(
        (s) =>
          ...
          (s.custom_branch ?? "").toLowerCase().includes(q)
      );
    }
    return list;
    ...
    >
      ...
      <div className="flex flex-col sm:flex-row sm:items-start sm:justify-between gap-3">
        <div>
          ...
          <p className="text-sm text-text-secondary mt-0.5">
            {isLoading
              ? "Loading…"
              : `${totalInRange} session${totalInRange !== 1 ? "s" : ""} ${
                  viewMode === "week" ? "this week" : "this month"
                }`}
          </p>
        </div>
        <Link href="/dashboard/instructor/course-schedule/new">
      ...
      <Card>
        <CardContent className="p-4 space-y-3">
          ...
          {/* Second row: search + batch filter + branch filter */}
          <div className="flex flex-col sm:flex-row gap-2">
            <div className="flex-1">
              <Input
                placeholder="Search course, group, program…"
      ...
      {/* ── Content: Week view or Calendar view ───────────────────────── */}
      {viewMode === "week" ? (
        <WeekView
          weekDates={weekDates}
          byDate={byDate}
          ...
          >
            ...
            >
              ...
              <p className="text-sm text-text-secondary">
                This will permanently remove the course schedule. This cannot be
                undone.
              </p>
          ...
          >
            <Card className={isToday ? "ring-1 ring-primary/40" : ""}>
              <CardContent className="p-0">
                ...
                {/* Sessions */}
                {sessions.length === 0 ? (
                  <div className="px-4 py-4 text-xs text-text-tertiary text-center">
                    —
                  </div>
                ) : (
        ...
        <CardContent className="p-3 sm:p-4">
          ...
          <div className="grid grid-cols-7 gap-1 mb-1">
            {DAY_LABELS.map((d) => (
              <div
                key={d}
                className="text-center text-[11px] font-semibold text-text-tertiary uppercase tracking-wider py-1"
              >
                {d}
              </div>
            ))}
            ...
            {calDates.map((date) => {
              ...
              return (
                <button
                  ...
                  className={`
                    relative min-h-[72px] sm:min-h-[84px] rounded-[8px] p-1.5 text-left
                  ...
                  {hasSchedules && (
                    <div className="mt-0.5 space-y-0.5">
                      {sessions.slice(0, 3).map((s) => {
                        const cls =
                          COLOR_BG_LIGHT[s.class_schedule_color ?? ""] ??
                        ...
                        return (
                          ...
                          >
                            {/* Desktop: course + instructor + time */}
      ...
      >
        ...
        {s.program && (
          <div className="flex items-center gap-1.5 text-xs text-text-secondary">
            ...
            <span className="truncate">{s.program}</span>
          </div>
        )}

        {/* Branch (shown in calendar detail panel) */}
        {showBranch && s.custom_branch && (
          <div className="flex items-center gap-1.5 text-xs text-text-secondary">
            <Building2 className="h-3 w-3 text-text-tertiary" />
            <span className="truncate">{s.custom_branch}</span>
          </div>
        )}
