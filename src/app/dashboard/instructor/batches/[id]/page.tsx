...
export default function InstructorBatchDetailPage() {
  ...
  useEffect(() => {
    // Wait for the instructor batches to load before checking access
    if (!batchesLoading) {
      loadBatch();
    }
  }, [decodedId, batchesLoading]);
    ...
    <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} className="space-y-6">
      ...
      {accessDenied && !loading && (
        <div className="flex flex-col items-center justify-center h-48 gap-3">
          ...
          <p className="text-error text-sm font-medium">Access Denied</p>
          <p className="text-text-secondary text-sm">This batch is not assigned to you.</p>
          <Button variant="outline" size="sm" onClick={() => router.push("/dashboard/instructor/batches")}>
            Back to My Batches
          </Button>
        </div>
      )}
        ...
        <>
          ...
          <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
            ...
            <div className="flex items-center gap-3">
              ...
              <Button variant="outline" size="md" onClick={loadBatch} disabled={loading}>
                    ...
                    <tbody>
                      {filteredStudents.map((student, index) => (
                        ...
                        >
                          ...
                          <td className="py-2.5">
                            <Badge variant={student.active === 1 ? "success" : "error"}>
