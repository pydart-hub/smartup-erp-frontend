...
import { Building2, UserCheck, ChevronRight, Users } from "lucide-react";
        ...
        {selectedBranch ? (
          ...
          <motion.div key="overview" initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0, x: -20 }} className="space-y-6">
            <div>
              ...
              <p className="text-sm text-text-tertiary mt-0.5">Instructor performance across all branches — click a branch to drill down</p>
            </div>

            <motion.div variants={container} initial="hidden" animate="show" className="grid grid-cols-2 sm:grid-cols-3 gap-3">
              {[
                { icon: <Users className="w-4 h-4 text-text-tertiary" />, label: "Total Instructors", value: totalInstructors, className: "text-primary" },
                { icon: <UserCheck className="w-4 h-4 text-success" />, label: "Avg Topic Completion", value: `${avgTopicCompletion}%`, className: pctColor(avgTopicCompletion, 70, 50) },
                { icon: <Building2 className="w-4 h-4 text-text-tertiary" />, label: "Avg Classes Done", value: `${avgClassesDone}%`, className: pctColor(avgClassesDone, 80, 60) },
              ].map((s) => (
                ...
                >
                  <div className="flex items-center gap-4 min-w-0">
                    ...
                    <div className="min-w-0">
                      ...
                      <p className="text-xs text-text-tertiary">{b.total_instructors ?? 0} instructors · Classes done: {safeNum(b.avg_classes_conducted_pct)}%</p>
                    </div>
                  </div>
                  <div className="flex items-center gap-4 shrink-0">
                    <div className="text-right hidden sm:block">
                      <p className="text-xs text-text-tertiary">Classes Done</p>
                      <p className={`text-sm font-bold ${pctColor(safeNum(b.avg_classes_conducted_pct), 80, 60)}`}>{safeNum(b.avg_classes_conducted_pct)}%</p>
                    </div>
