...
import { Building2, BookOpen, ChevronRight, BookMarked } from "lucide-react";
...
export default function DirectorAcademicsTopicCoveragePage() {
  ...
  const overallTopicCoverage = branches.length ? Math.round(branches.reduce((a, b) => a + safeNum(b.topic_coverage_pct), 0) / branches.length) : 0;
  const avgInstructorTopic = branches.length ? Math.round(branches.reduce((a, b) => a + safeNum(b.avg_instructor_topic_pct), 0) / branches.length) : 0;

  return (
    <div className="p-4 sm:p-6 space-y-6 max-w-7xl mx-auto">
      <AnimatePresence mode="wait">
        {selectedBranch ? (
          <BranchDrillDown key={selectedBranch} branch={selectedBranch} onBack={() => setSelectedBranch(null)} defaultTab="events" />
        ) : (
          <motion.div key="overview" initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0, x: -20 }} className="space-y-6">
            ...
            <motion.div initial={{ opacity: 0, y: -12 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.5, ease: [0.22,1,0.36,1] }}
              ...
              <div>
                <h1 className="text-lg font-bold text-text-primary tracking-tight">Topic Coverage Overview</h1>
                <p className="text-xs text-text-secondary">Curriculum progress across all branches — click a branch to drill down</p>
              </div>
            </motion.div>

            {/* ── Stat Cards ── */}
            <div className="grid grid-cols-1 sm:grid-cols-3 gap-3">
              <StatCard3D icon={Building2} iconClass="bg-brand-wash text-primary" label="Branches" value={branches.length} delay={0.05} />
              <StatCard3D icon={BookOpen} iconClass="bg-success/10 text-success" label="Avg Topic Coverage" delay={0.1}
                value={<span className={pctColor(overallTopicCoverage, 70, 50)}>{overallTopicCoverage}%</span>} />
              <StatCard3D icon={BookMarked} iconClass="bg-primary-light text-primary" label="Avg Instructor Topics" delay={0.15}
                value={<span className={pctColor(avgInstructorTopic, 70, 50)}>{avgInstructorTopic}%</span>} />
            </div>

            {/* ── Branch Rows ── */}
            <div className="space-y-2">
              {sorted.map((b, i) => {
                const coveragePct = safeNum(b.topic_coverage_pct);
                const instrPct = safeNum(b.avg_instructor_topic_pct);
                return (
                  <TopicBranchRow key={b.branch} branch={b.branch} coveragePct={coveragePct} instrPct={instrPct}
                    batches={b.total_batches} delay={i * 0.04} onClick={() => setSelectedBranch(b.branch)} />
                );
              })}
            </div>
                    ...
                    <table className="w-full text-sm">
                      <thead>
                        <tr className="bg-app-bg border-b border-border-light">
                          {["Branch","Topic Coverage","Instructor Topics","Batches","Teachers"].map((h, hi) => (
...
function TopicBranchRow({ branch, coveragePct, instrPct, batches, delay, onClick }: {
  branch: string; coveragePct: number; instrPct: number; batches: number; delay: number; onClick: () => void;
}) {
  ...
  const onLeave = () => { mx.set(0); my.set(0); };
  const instrColor = instrPct >= 70 ? "text-success" : instrPct >= 50 ? "text-warning" : "text-error";
  return (
    <motion.div initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }}
      ...
      <motion.button ref={ref} onClick={onClick} onMouseMove={onMove} onMouseLeave={onLeave}
        ...
        <div className={`w-12 h-12 rounded-xl flex items-center justify-center text-sm font-black shrink-0 ${pctBadgeColor(coveragePct, 70, 50)}`}>
          {coveragePct}%
        </div>
        <div className="flex-1 min-w-0">
          <p className="text-sm font-semibold text-text-primary truncate">{branch.replace("Smart Up ", "")}</p>
          <p className="text-xs text-text-tertiary mt-0.5">Instructor topics: <span className={instrColor}>{instrPct}%</span> · {batches} batch{batches !== 1 ? "es" : ""}</p>
        </div>
        <div className="hidden sm:flex flex-col items-end shrink-0">
          <span className="text-[10px] font-semibold uppercase tracking-wider text-text-tertiary">Instructor Topics</span>
          <span className={`text-sm font-black ${instrColor}`}>{instrPct}%</span>
        </div>
