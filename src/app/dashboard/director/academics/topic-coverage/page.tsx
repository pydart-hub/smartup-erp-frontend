        ...
        {selectedBranch ? (
          ...
          <motion.div key="overview" initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0, x: -20 }} className="space-y-6">
            <div>
              ...
              <p className="text-sm text-text-tertiary mt-0.5">Curriculum progress across all branches — click a branch to drill down</p>
            </div>
            ...
            <motion.div variants={container} initial="hidden" animate="show" className="grid grid-cols-2 sm:grid-cols-3 gap-3">
              {[
                ...
                { icon: <BookOpen className="w-4 h-4 text-success" />, label: "Avg Topic Coverage", value: `${overallTopicCoverage}%`, className: pctColor(overallTopicCoverage, 70, 50) },
                { icon: <BookOpen className="w-4 h-4 text-text-tertiary" />, label: "Avg Instructor Topics", value: `${avgInstructorTopic}%`, className: pctColor(avgInstructorTopic, 70, 50) },
              ].map((s) => (
                ...
                >
                  <div className="flex items-center gap-4 min-w-0">
                    ...
                    <div className="min-w-0">
                      ...
                      <p className="text-xs text-text-tertiary">Instructor topics: {safeNum(b.avg_instructor_topic_pct)}% · {b.total_batches} batches</p>
                    </div>
                  </div>
                  <div className="flex items-center gap-4 shrink-0">
                    <div className="text-right hidden sm:block">
                      <p className="text-xs text-text-tertiary">Instructor Topics</p>
                      <p className={`text-sm font-bold ${pctColor(safeNum(b.avg_instructor_topic_pct), 70, 50)}`}>{safeNum(b.avg_instructor_topic_pct)}%</p>
                    </div>
                      ...
                      <tbody>
                        {sorted.map((b) => (
                          <tr key={b.branch} onClick={() => setSelectedBranch(b.branch)} className="border-b border-border-light last:border-0 hover:bg-app-bg transition-colors cursor-pointer">
                            ...
                            <td className="p-3 text-center"><span className={`inline-flex px-2 py-0.5 rounded-full text-xs font-bold ${pctBadgeColor(safeNum(b.topic_coverage_pct), 70, 50)}`}>{safeNum(b.topic_coverage_pct)}%</span></td>
                            <td className="p-3 text-center"><span className={`inline-flex px-2 py-0.5 rounded-full text-xs font-bold ${pctBadgeColor(safeNum(b.avg_instructor_topic_pct), 70, 50)}`}>{safeNum(b.avg_instructor_topic_pct)}%</span></td>
