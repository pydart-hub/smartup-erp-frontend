        ...
        {selectedBranch ? (
          ...
          <motion.div key="overview" initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0, x: -20 }} className="space-y-6">
            <div>
              ...
              <p className="text-sm text-text-tertiary mt-0.5">Cross-branch exam performance — click a branch to drill down</p>
            </div>
            ...
            <motion.div variants={container} initial="hidden" animate="show" className="grid grid-cols-2 sm:grid-cols-4 gap-3">
              {[
                ...
                { icon: <ClipboardList className="w-4 h-4 text-text-tertiary" />, label: "Total Exams", value: totalExams, className: "text-primary" },
                { icon: <BarChart3 className="w-4 h-4 text-text-tertiary" />, label: "Avg Score", value: `${overallAvgScore}%`, className: pctColor(overallAvgScore, 60, 40) },
                { icon: <Trophy className="w-4 h-4 text-warning" />, label: "Pass Rate", value: `${overallPassRate}%`, className: pctColor(overallPassRate) },
              ].map((s) => (
                      ...
                      <tbody>
                        {sorted.map((b) => (
                          <tr key={b.branch} onClick={() => setSelectedBranch(b.branch)} className="border-b border-border-light last:border-0 hover:bg-app-bg transition-colors cursor-pointer">
                            ...
                            <td className="p-3 text-center"><span className={`inline-flex px-2 py-0.5 rounded-full text-xs font-bold ${pctBadgeColor(safeNum(b.avg_exam_score_pct), 60, 40)}`}>{safeNum(b.avg_exam_score_pct)}%</span></td>
                            <td className="p-3 text-center"><span className={`inline-flex px-2 py-0.5 rounded-full text-xs font-bold ${pctBadgeColor(safeNum(b.pass_rate))}`}>{safeNum(b.pass_rate)}%</span></td>
