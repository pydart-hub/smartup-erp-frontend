...
import { getDirectorMentorStudentDetail } from "@/lib/api/mentors";
...
export default function DirectorMentorStudentDetailPage() {
  const { id } = useParams<{ id: string }>();
  const studentId = decodeURIComponent(id);
  const [attendanceMode, setAttendanceMode] = useState<"summary" | "absent">("summary");

  const { data, isLoading, error } = useQuery({
    queryKey: ["director-mentor-student-detail", studentId],
    queryFn: () => getDirectorMentorStudentDetail(studentId),
    staleTime: 30_000,
  });
        ...
        <CardContent className="relative p-0">
          ...
          <div className="relative grid gap-6 p-6 lg:grid-cols-[1.4fr_0.9fr] lg:p-8">
            <div className="space-y-5">
              <div>
                <Link href="/dashboard/director/mentor-feedback" className="inline-flex items-center gap-1.5 text-xs font-semibold text-primary hover:underline mb-3">
                ...
                <div className="flex items-start gap-4 flex-wrap">
                  ...
                  <div className="min-w-0 flex-1">
                    ...
                    <p className="mt-3 max-w-2xl text-sm leading-6 text-slate-600">
                      Director view of the complete mentor-linked student profile with academic, attendance, fee, parent, and feedback visibility.
                    </p>
                  </div>
                </div>
              </div>
