...
function BranchTeacherRow({ branch }: { branch: { name: string; abbr: string } }) {
  ...
  return (
    <Link href={`/dashboard/director/branches/${encodeURIComponent(branch.name)}/teachers`}>
  ...
  return (
    <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} className="space-y-6">
      ...
      <div>
        ...
        <p className="text-sm text-text-secondary mt-0.5">Select a branch to view instructors</p>
      </div>
      ...
      {isLoading ? (
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
          {filtered.map((branch) => (
            <BranchTeacherRow key={branch.name} branch={branch} />
          ))}
        </div>
      )}
    </motion.div>
