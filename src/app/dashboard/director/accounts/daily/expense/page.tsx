...
function DailyExpenseDetailsInner() {
  ...
  // Fetch Expenses
  const { data: expenses, isLoading } = useQuery<DailyExpenseItem[]>({
    queryKey: ["director-today-expenses", fromDate, toDate],
    queryFn: async () => {
      const res = await fetch(`/api/director/today-expenses?from_date=${fromDate}&to_date=${toDate}`, {
        credentials: "include",
      });
  ...
  const totalExpense = useMemo(() => {
    return (expenses ?? []).reduce((sum, item) => sum + (item.debit ?? 0), 0);
  }, [expenses]);

  // Group by branch
  const branchGroups = useMemo(() => {
    const groups: Record<string, { total: number; items: DailyExpenseItem[] }> = {};
    for (const e of expenses ?? []) {
      const branchName = e.company.replace("Smart Up ", "").replace("Smart Up", "HQ");
      if (!groups[branchName]) {
        groups[branchName] = { total: 0, items: [] };
      }
      groups[branchName].total += e.debit;
      groups[branchName].items.push(e);
    }
  ...
  const handleFromDateChange = (date: string) => {
    setFromDate(date);
    router.replace(`/dashboard/director/accounts/daily/expense?from_date=${date}&to_date=${toDate}`);
  };

  const handleToDateChange = (date: string) => {
    setToDate(date);
    router.replace(`/dashboard/director/accounts/daily/expense?from_date=${fromDate}&to_date=${date}`);
  };
    ...
    <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} className="space-y-6">
        ...
        <div className="flex items-center gap-3">
          ...
          <div>
            <h1 className="text-2xl font-bold text-text-primary flex items-center gap-2">
              <Receipt className="h-6 w-6 text-rose-600" /> Daily Expenses by Branch
            </h1>
      ...
      {isLoading ? (
        <div className="flex flex-col items-center justify-center py-20 gap-3">
          <GifLoader size="lg" />
          <p className="text-sm text-text-secondary">Loading branch expenses...</p>
        </div>
      ) : (
        <div className="space-y-4">
          ...
          <div className="rounded-2xl border border-border-light bg-surface p-6 flex items-center justify-between shadow-sm">
            <div className="flex items-center gap-4">
              ...
              <div>
                <p className="text-sm text-text-tertiary">Grand Total Expenses</p>
                <p className="text-3xl font-extrabold text-rose-600 mt-0.5">
            ...
            {branchGroups.length > 0 ? (
              branchGroups.map(([branchName, data]) => {
                ...
                return (
                  <div
                    key={branchName}
                    className="border border-border-light rounded-2xl bg-surface overflow-hidden shadow-sm hover:border-rose-500/10 transition-colors"
                  >
                    ...
                    >
                      <div className="flex items-center gap-3">
                        <div className="w-10 h-10 rounded-lg bg-rose-500/5 flex items-center justify-center border border-rose-500/10 shrink-0">
                          <Receipt className="h-5 w-5 text-rose-600" />
                        </div>
                        <div>
                          <p className="text-sm font-bold text-text-primary">{branchName}</p>
                          <p className="text-xs text-text-tertiary mt-0.5">{data.items.length} expense entries</p>
                        </div>
                      </div>
                            ...
                            <table className="w-full text-left border-collapse">
                              <thead>
                                <tr className="border-b border-border-light text-[10px] uppercase font-bold text-text-tertiary bg-background/50">
                                  ...
                                  <th className="px-4 py-3">Voucher Reference</th>
                              ...
                              <tbody className="divide-y divide-border-light/40 text-xs text-text-primary">
                                {data.items.map((item) => (
                                  <tr key={item.name} className="hover:bg-background/20 transition-colors">
                                    ...
                                    <td className="px-4 py-3 font-mono text-[10px] text-text-tertiary">{item.voucher_no}</td>
                                    <td className="px-4 py-3 text-text-tertiary max-w-[300px] truncate" title={item.remarks || ""}>
                                      {item.remarks || "-"}
                                    </td>
                                    <td className="px-4 py-3 text-right font-bold text-rose-600">
                                      {formatCurrencyExact(item.debit)}
                                    </td>
                                  </tr>
                                ))}
                              </tbody>
              ...
              <div className="py-12 text-center bg-surface border border-border-light rounded-2xl shadow-sm">
                <p className="text-sm text-text-tertiary">No expenses recorded for this date.</p>
              </div>
            )}
          </div>
        </div>
