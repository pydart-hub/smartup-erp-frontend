...
export default async function DirectorStudentResultDetailPage({ params }: PageProps) {
  ...
  try {
    ...
    return (
      <div className="p-4 lg:p-6 max-w-4xl mx-auto space-y-6 text-slate-800">
        ...
        <div className="bg-white border border-slate-200 rounded-3xl p-6 shadow-sm space-y-4 relative">
          ...
          <h3 className="text-lg font-bold text-slate-955">{aiSummary.headline}</h3>
    ...
    console.error("Database connection error in detailed view (Director):", error);
    return <DatabaseErrorCard error={error} backUrl="/dashboard/director/diagnosis-exams" />;
  }
}
