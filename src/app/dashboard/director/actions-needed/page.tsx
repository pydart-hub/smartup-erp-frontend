...
export default function DirectorActionsNeededPage() {
  ...
  const { data, isLoading } = useQuery({
    queryKey: ["director-branch-actions-needed", selectedWeekStart],
      ...
      <AnimatePresence mode="wait">
        <motion.div key="overview" initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0, x: -20 }} className="space-y-6">
          <motion.div
            ...
            className="relative overflow-hidden rounded-[14px] border border-primary/15 bg-gradient-to-r from-primary/5 via-info/5 to-warning/5 p-4"
          >
            <motion.div
              className="absolute -top-12 -right-8 h-28 w-28 rounded-full bg-primary/10 blur-2xl"
              animate={{ x: [0, 8, 0], y: [0, -6, 0] }}
              transition={{ duration: 6, repeat: Infinity, ease: "easeInOut" }}
            />
            <motion.div
              className="absolute -bottom-10 left-10 h-24 w-24 rounded-full bg-warning/10 blur-2xl"
              animate={{ x: [0, -8, 0], y: [0, 6, 0] }}
              transition={{ duration: 7, repeat: Infinity, ease: "easeInOut" }}
            />
            <div className="relative">
              <div className="inline-flex items-center gap-1.5 px-2.5 py-1 rounded-full bg-surface border border-border-light text-xs text-primary mb-2">
                <Sparkles className="w-3.5 h-3.5" /> Weekly Focus
              </div>
              <h1 className="text-2xl font-bold text-primary">Actions Needed</h1>
              <p className="text-sm text-text-tertiary mt-0.5">Director view of branch-wise weekly schedule and attendance follow-ups</p>
            </div>
            ...
            <div className="mt-3 flex flex-wrap items-center gap-2">
              ...
              <div className="flex items-center gap-1.5 px-3 py-1.5 rounded-lg border border-border-light bg-surface text-sm font-medium text-text-primary min-w-[220px] justify-center">
                ...
                <span>{weekFrom && weekTo ? `${weekFrom} → ${weekTo}` : selectedWeekStart}</span>
              </div>
              <button
                type="button"
                onClick={() => { setWeekOffset((v) => v + 1); setHolidayAdjustment(0); }}
            ...
            <div className="mt-2 flex flex-wrap items-center gap-2 text-xs text-text-tertiary">
              <span>(Sunday excluded, public holidays: {publicHolidayDays})</span>
              <button
                type="button"
          ...
          <motion.div variants={container} initial="hidden" animate="show" className="grid grid-cols-2 sm:grid-cols-4 gap-3">
            ...
            <motion.div variants={item} className="bg-surface rounded-[12px] p-4 border border-border-light">
              ...
              <p className="text-[11px] text-text-tertiary mt-1">Sum of (Not Scheduled + Attendance Not Marked) across visible branches</p>
            </motion.div>
            <motion.div variants={item} className="bg-surface rounded-[12px] p-4 border border-border-light">
              <div className="flex items-center gap-2 mb-2">
          ...
          <motion.div variants={container} initial="hidden" animate="show" className="space-y-2">
            {sorted.map(({ branch: b, metrics }) => (
              <motion.button
                ...
                onClick={() => router.push(`/dashboard/director/actions-needed/${encodeURIComponent(b.branch)}`)}
