...
export default function DirectorActionNeededBranchDetailPage() {
  const params = useParams<{ branch: string }>();
  const branch = decodeURIComponent(params?.branch ?? "");

  const { data, isLoading } = useQuery({
    queryKey: ["director-branch-actions-needed-detail", branch],
  ...
  return (
    <div className="p-4 sm:p-6 space-y-6 max-w-6xl mx-auto">
      <div className="space-y-2">
        <Link
          href="/dashboard/director/actions-needed"
