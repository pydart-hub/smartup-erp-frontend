...
function PodiumCard({
  ...
  const [hoverRotate, setHoverRotate] = React.useState({ x: 0, y: 0 });
  ...
  return (
    <motion.div
      ...
      onMouseLeave={() => setHoverRotate({ x: 0, y: 0 })}
    >
      <motion.div
        whileHover={{ y: -8, scale: 1.02, boxShadow: "0 40px 80px rgba(0,0,0,0.25)" }}
        ...
        className={`relative rounded-2xl border ${p.border} ${p.ring} ${p.glow}
          bg-white/80 dark:bg-slate-900/80 backdrop-blur-xl overflow-hidden transition-all`}
      >
        ...
        {isFirst && (
          <motion.div
            animate={{ opacity: [0.5, 1, 0.5], scale: [1, 1.2, 1], y: [0, -2, 0] }}
            transition={{ repeat: Infinity, duration: 2.5, ease: "easeInOut" }}
            className="absolute -top-8 left-1/2 -translate-x-1/2 w-32 h-32 rounded-full pointer-events-none blur-2xl"
            style={{ background: `radial-gradient(circle, ${p.glowColor}, transparent 60%)` }}
          />
        )}

        {/* Dual corner glows */}
        <div
          className="absolute -top-4 -right-4 w-24 h-24 rounded-full blur-2xl pointer-events-none opacity-60"
          style={{ background: `radial-gradient(circle, ${p.glowColor}, transparent)` }}
        />
        <div
          className="absolute -bottom-4 -left-4 w-20 h-20 rounded-full blur-2xl pointer-events-none opacity-40"
          style={{ background: `radial-gradient(circle, ${p.glowColor}, transparent)` }}
        />
        ...
        <div className="relative p-5">
          ...
          <div className="flex items-start gap-3 mb-4">
            <motion.div
              animate={isFirst ? { rotate: [0, -12, 12, -6, 0], y: [0, -1, 1, -0.5, 0] } : { scale: [1, 1.05, 1] }}
              transition={{ repeat: Infinity, duration: isFirst ? 3 : 2.5, ease: "easeInOut" }}
              className={`w-10 h-10 rounded-xl bg-gradient-to-br ${p.badgeBg} flex items-center justify-center shadow-lg shadow-current/25 shrink-0 ring-2 ring-white/20`}
            >
              <RankIcon className="w-5 h-5 text-white drop-shadow-md" />
            </motion.div>
...
function RankRow({
  ...
  const rankStyle =
    rank === 1
      ? "bg-gradient-to-br from-yellow-400 to-amber-500 text-white shadow-[0_4px_12px_rgba(234,179,8,0.5)] ring-1 ring-white/20"
      : rank === 2
      ? "bg-gradient-to-br from-slate-300 to-slate-500 text-white shadow-[0_4px_12px_rgba(148,163,184,0.3)] ring-1 ring-white/20"
      : rank === 3
      ? "bg-gradient-to-br from-orange-400 to-amber-600 text-white shadow-[0_4px_12px_rgba(249,115,22,0.4)] ring-1 ring-white/20"
  ...
  return (
    <motion.div
      ...
      className="border-b border-black/[0.05] dark:border-white/[0.05] last:border-0"
    >
      <motion.button
        onClick={onToggle}
        whileHover={{ backgroundColor: "rgba(0,0,0,0.02)" }}
        whileTap={{ scale: 0.98 }}
        className="w-full flex items-center gap-3 px-4 py-3.5 hover:bg-gradient-to-r hover:from-primary/[0.08] hover:to-transparent transition-all text-left group"
      >
        <motion.div
          whileHover={{ scale: 1.2, rotate: [0, -5, 5, 0] }}
          whileTap={{ scale: 0.9 }}
          className={`w-7 h-7 rounded-lg flex items-center justify-center text-xs font-black shrink-0 ${rankStyle} transition-all`}
        >
          {rank}
        </motion.div>
      ...
      >
        <div>
          ...
          <div className="flex items-center gap-3 mt-2">
            <motion.div
              animate={{ rotate: [0, -15, 15, -7, 0], y: [0, -3, 0] }}
              transition={{ repeat: Infinity, duration: 3.5, ease: "easeInOut" }}
              className="w-11 h-11 rounded-xl bg-gradient-to-br from-yellow-400 via-amber-400 to-amber-500 flex items-center justify-center shadow-lg shadow-amber-400/50 ring-2 ring-white/20"
              style={{
                boxShadow: "0 0 20px rgba(234, 179, 8, 0.4), 0 8px 20px rgba(234, 179, 8, 0.25)",
              }}
            >
      ...
      {sorted.length > 0 && (
        <div>
          <motion.p
            ...
            className="text-[10px] font-black uppercase tracking-widest text-text-tertiary mb-4 flex items-center gap-2"
          >
      ...
      {sorted.length > 0 && (
        <div>
          ...
          <motion.div
            ...
            className="bg-white/50 dark:bg-white/[0.04] backdrop-blur-lg rounded-2xl border border-white/70 dark:border-white/10 overflow-hidden shadow-[0_8px_32px_rgba(0,0,0,0.1)]"
          >
            {sorted.map((entry, i) => (
              <RankRow
                key={entry.instructor}
