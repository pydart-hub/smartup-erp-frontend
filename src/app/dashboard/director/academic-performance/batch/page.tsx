...
function BatchSubjectContent() {
  ...
  const { data, isLoading } = useQuery<ClassPerformanceResponse>({
    queryKey: ["director-batch-subject", branch, program, batch],
  ...
  return (
    <div className="p-4 sm:p-6 lg:p-8 max-w-7xl mx-auto space-y-6 pb-16">
      ...
      <div className="flex items-center gap-2 text-xs text-text-tertiary flex-wrap">
        ...
        <button
          onClick={() => router.push(`/dashboard/director/academic-performance/class?branch=${encodeURIComponent(branch)}&program=${encodeURIComponent(program)}`)}
      ...
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4 pb-2 border-b border-border-light">
        ...
        <button
          onClick={() => router.push(`/dashboard/director/academic-performance/class?branch=${encodeURIComponent(branch)}&program=${encodeURIComponent(program)}&batch=${encodeURIComponent(batch)}`)}
...
export default function BatchSubjectPage() {
  return (
    <React.Suspense
      fallback={
