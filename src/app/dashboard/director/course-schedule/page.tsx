...
function BranchScheduleRow({ branch }: { branch: { name: string; abbr: string } }) {
      ...
      <div className="flex items-center gap-3 p-3 rounded-[10px] border border-border-light hover:border-primary/30 hover:shadow-sm transition-all cursor-pointer bg-surface">
        ...
        <div className="text-right shrink-0">
          <p className="text-lg font-bold text-text-primary">
            {isLoading ? <span className="inline-block w-8 h-5 bg-border-light rounded animate-pulse" /> : (count ?? 0)}
          </p>
          <p className="text-[10px] text-text-tertiary">schedules</p>
        </div>
...
export default function DirectorCourseSchedulePage() {
  ...
  return (
    <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} className="space-y-6">
      <BreadcrumbNav />
      <div>
        <h1 className="text-2xl font-bold text-text-primary">Course Schedule</h1>
        <p className="text-sm text-text-secondary mt-0.5">Select a branch to view its schedule</p>
      </div>
      ...
      {isLoading ? (
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
          {filtered.map((branch) => (
            <BranchScheduleRow key={branch.name} branch={branch} />
          ))}
        </div>
      )}
    </motion.div>
