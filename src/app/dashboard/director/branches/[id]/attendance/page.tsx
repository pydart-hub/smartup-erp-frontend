...
export default function BranchAttendancePage() {
  ...
  const {
    ...
    queryKey: ["director-branch-attendance", branchName, selectedDate],
    ...
    >
      ...
      {isLoading ? (
        ...
        <div className="flex flex-col items-center justify-center h-48">
          ...
          <p className="text-sm text-text-tertiary">No attendance records for this date</p>
        </div>
      ) : (
        <div className="space-y-4">
          {Object.entries(byGroup)
            ...
            .map(([group, recs]) => {
              const gPresent = recs.filter((r) => r.status === "Present").length;
              const total = recs.length;
              const pct = total > 0 ? Math.round((gPresent / total) * 100) : 0;
              return (
                <Card key={group}>
