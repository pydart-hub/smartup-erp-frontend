  ...
  return (
    <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} className="space-y-6">
      ...
      <div>
        ...
        <p className="text-sm text-text-secondary mt-0.5">Select a branch to view attendance records</p>
      </div>
      ...
      {isLoading ? (
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
          {filtered.map((branch) => {
            ...
            return (
              <Link key={branch.name} href={`/dashboard/director/branches/${encodeURIComponent(branch.name)}/attendance`}>
