...
function BranchCard({ branch, index, isDark }: {
  ...
  const href = `/dashboard/director/attendance/students/${encodeURIComponent(branch.name)}`;
...
// ── Page ───────────────────────────────────────────────────────────────────
export default function DirectorStudentAttendancePage() {
      ...
      <div className="relative z-10 px-6 sm:px-8 py-8 max-w-5xl mx-auto">
        ...
        >
          <Link
            ...
            className={`inline-flex items-center gap-1.5 text-sm font-medium transition-colors ${
              isDark ? "text-white/45 hover:text-white/80" : "text-gray-400 hover:text-teal-700"
            }`}
          >
        ...
        >
          ...
          <p className={`mt-2 text-sm max-w-md leading-relaxed ${isDark ? "text-white/45" : "text-gray-500"}`}>
            Select a branch to view student attendance records.
          </p>
        </motion.div>
