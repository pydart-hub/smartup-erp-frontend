...
import { Search, Building2, LayoutGrid, ChevronRight, AlertCircle } from "lucide-react";
import { getAllBranches, getBatchCountForBranch } from "@/lib/api/director";

// ── Per-branch accent palette ─────────────────────────────────────────────────
const ACCENTS = [
...
function ThreeScene({ isDark }: { isDark:boolean }) {
  return (
    <>
      ...
      <FloatingShape position={[-8,2.5,-4]}  shape="icosa" color={isDark?"#1A9E8F":"#0D9488"} speed={0.25} opacity={isDark?0.45:0.16} />
...
function BranchCard({ branch, index, isDark }: {
  ...
  const href        = `/dashboard/director/branches/${encodeURIComponent(branch.name)}/batches`;
          ...
          <div className="p-5 flex items-center gap-4">
            ...
            <div className="flex-1 min-w-0" style={{ transform:"translateZ(8px)" }}>
              <p className={`text-sm font-semibold truncate transition-colors ${
                isDark ? "text-white/90 group-hover:text-white" : "text-gray-800 group-hover:text-teal-700"
              }`}>{shortName}</p>
              <div className={`inline-flex items-center mt-1 px-1.5 py-0.5 rounded text-[10px] font-mono font-medium ${
      ...
      <div className="relative z-10 px-6 sm:px-8 py-8 max-w-5xl mx-auto">
        ...
        >
          ...
          <p className={`mt-2 text-sm max-w-md leading-relaxed ${isDark?"text-white/45":"text-gray-500"}`}>
            Select a branch to view its batches and classes.
          </p>
        </motion.div>
        ...
        {isLoading ? (
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
            {Array.from({ length:8 }).map((_,i) => (
              <div key={i} className={`h-[76px] rounded-2xl animate-pulse ${isDark?"bg-white/[0.05]":"bg-white/80"}`}
