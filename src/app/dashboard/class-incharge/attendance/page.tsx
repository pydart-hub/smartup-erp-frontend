...
import {
  ...
  BookOpen, ChevronDown, GraduationCap, Bell,
} from "lucide-react";
import { AbsentDetailPanel } from "./AbsentDetailPanel";
...
export default function ClassInchargeAttendancePage() {
  ...
  // Derive branch from allowed batches when defaultCompany is missing.
  // Class-incharge instructors may not have Company user permissions, so we
  // look up the branch from their first Student Batch Name permission instead.
  const firstBatchCode = allowedBatches?.[0] ?? "";
  const { data: derivedBranch } = useQuery({
    queryKey: ["ci-derive-branch", firstBatchCode],
    queryFn: async () => {
  ...
  const [isAbsentPanelOpen, setIsAbsentPanelOpen] = useState(false);

  // ── Batches ──
  const { data: batchesRes } = useQuery({
    queryKey: ["ci-batches-list", effectiveBranch, selectedYear],
    queryFn: () =>
      getBatches({
        limit_page_length: 500,
        ...(effectiveBranch ? { custom_branch: effectiveBranch } : {}),
  ...
  const batchMap = useMemo(() => new Map(batches.map((b: Batch) => [b.name, b])), [batches]);

  // ── O2O groups ──
  const { data: o2oGroupRes } = useQuery({
    queryKey: ["ci-o2o-groups", effectiveBranch],
    queryFn: () => getStudentGroups({ branch: effectiveBranch || undefined, oneToOneOnly: true }),
    staleTime: 5 * 60_000,
    enabled: !!effectiveBranch,
  });
  ...
  const { data: branchAttendanceRes, isLoading: classWiseLoading } = useQuery({
    queryKey: ["ci-branch-attendance", effectiveBranch, selectedDate],
    queryFn: () => getAttendance(selectedDate, { custom_branch: effectiveBranch || undefined }),
    staleTime: 30_000,
    enabled: !!effectiveBranch && viewMode === "dashboard",
  });

  // ── Batch member lists ──
  const { data: batchMembersMap } = useQuery({
    queryKey: ["ci-batch-members", effectiveBranch, selectedYear],
    queryFn: async () => {
    ...
    staleTime: 5 * 60_000,
    enabled: !!effectiveBranch && batches.length > 0 && viewMode === "dashboard",
  });

  // ── Schedules for session counts ──
  const { data: branchSchedulesRes } = useQuery({
    queryKey: ["ci-branch-schedules", effectiveBranch, selectedDate],
    queryFn: () =>
      getCourseSchedules({ branch: effectiveBranch || undefined, date: selectedDate, limit_page_length: 500 }),
    staleTime: 60_000,
    enabled: !!effectiveBranch && viewMode === "dashboard",
  });
  ...
  // ── Batch session detail queries ──
  const { data: upcomingSchedulesRes } = useQuery({
    queryKey: ["ci-batch-upcoming", selectedBatchId],
    queryFn: () =>
      getCourseSchedules({
  ...
  const { data: batchSchedulesRes, isLoading: batchSchedulesLoading } = useQuery({
    queryKey: ["ci-batch-sessions", selectedBatchId, selectedDate],
    queryFn: () =>
      getCourseSchedules({ date: selectedDate, student_group: selectedBatchId || undefined, limit_page_length: 50 }),
    enabled: !!selectedBatchId && viewMode === "batch",
    staleTime: 60_000,
  });
  ...
  const { data: batchAttendanceCount } = useQuery({
    queryKey: ["ci-batch-att-check", selectedBatchId, selectedDate],
    queryFn: async () => {
  ...
  useEffect(() => {
    ...
    if (!ids.length) { setDisabilityMap({}); return; }
    apiClient
  ...
  async function saveSessionAttendance(schedule: CourseSchedule) {
    ...
    try {
      ...
      toast.success(`Attendance saved for ${schedule.custom_topic || schedule.course}`);
      setIsAbsentPanelOpen(true);

      queryClient.invalidateQueries({ queryKey: ["ci-batch-att-check", selectedBatchId, selectedDate] });
      queryClient.invalidateQueries({ queryKey: ["ci-batch-sessions", selectedBatchId, selectedDate] });
      queryClient.invalidateQueries({ queryKey: ["ci-branch-attendance", effectiveBranch, selectedDate] });
  ...
  return (
    <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} className="space-y-6">
      ...
      <AbsentDetailPanel
        isOpen={isAbsentPanelOpen}
        onClose={() => setIsAbsentPanelOpen(false)}
      ...
      <AnimatePresence mode="wait">
        {viewMode === "dashboard" ? (
          <motion.div key="dashboard" initial={{ opacity: 0, x: -20 }} animate={{ opacity: 1, x: 0 }} exit={{ opacity: 0, x: -20 }} className="space-y-6">
            ...
            <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
              <div>
                ...
                <p className="text-sm text-text-secondary mt-0.5">Class-wise attendance overview</p>
              </div>
              <div className="flex items-end gap-3">
                ...
                <button
                  type="button"
                  onClick={() => setIsAbsentPanelOpen(true)}
                  className="h-10 flex items-center gap-1.5 px-3.5 rounded-[10px] border border-error/30 bg-error/5 text-error text-sm font-medium hover:bg-error/10 transition-colors shrink-0"
                >
            ...
            <div className="grid grid-cols-2 sm:grid-cols-4 gap-4">
              <Card><CardContent className="p-4 text-center"><p className="text-3xl font-bold text-primary">{overallPct}%</p><p className="text-xs text-text-secondary font-medium mt-1">Overall Attendance</p></CardContent></Card>
              <Card><CardContent className="p-4 text-center"><p className="text-3xl font-bold text-success">{overallPresent}</p><p className="text-xs text-success font-medium mt-1">Present</p></CardContent></Card>
              <Card><CardContent className="p-4 text-center"><p className="text-3xl font-bold text-error">{overallAbsent}</p><p className="text-xs text-error font-medium mt-1">Absent</p></CardContent></Card>
              <Card><CardContent className="p-4 text-center"><p className="text-3xl font-bold text-warning">{overallLate}</p><p className="text-xs text-warning font-medium mt-1">Late</p></CardContent></Card>
            </div>
            ...
            {classWiseLoading ? (
              ...
              <div className="text-center py-16 text-text-secondary text-sm">No batches found for this branch.</div>
            ) : (
              <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4">
                {classSummaries.map((cls, index) => {
                  const hasData = cls.total > 0;
                  return (
                    <motion.div key={cls.name} initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: index * 0.04 }}>
                      <Card className="cursor-pointer hover:shadow-card-hover hover:border-primary/20 transition-all group" onClick={() => openClassAttendance(cls.name)}>
                        <CardContent className="p-5">
                          <div className="flex items-center justify-between mb-4">
                            <div className="flex items-center gap-2">
                              ...
                              <div>
                                <h3 className="font-semibold text-text-primary text-sm">{cls.displayName}</h3>
                          ...
                          {hasData ? (
                            <>
                              ...
                              <div className="flex items-center justify-between text-xs">
                                ...
                                <Badge variant={cls.percentage >= 80 ? "success" : cls.percentage >= 60 ? "warning" : "error"} className="text-[10px]">{cls.percentage}%</Badge>
                              </div>
                            </>
                          ) : (
            ...
            {/* Subject-wise collapsible */}
            {subjectSummaries.length > 0 && (
              <div className="space-y-3">
                <button type="button" onClick={() => setSubjectExpanded((v) => !v)} className="w-full text-left">
                  <Card className="hover:shadow-card-hover hover:border-primary/20 transition-all">
                    <CardContent className="p-5">
                      <div className="flex items-center justify-between">
                        <div className="flex items-center gap-3">
                          <div className="w-9 h-9 rounded-[10px] bg-indigo-50 flex items-center justify-center"><BookOpen className="h-4 w-4 text-indigo-600" /></div>
                          <div>
                        ...
                        <div className="flex items-center gap-3">
                          {subjectSummaries.some((s) => s.total > 0) && (
                            <div className="flex items-center gap-2 text-xs">
                              <span className="flex items-center gap-1 text-success font-medium"><CheckCircle className="h-3 w-3" />{subjectSummaries.reduce((s, c) => s + c.present, 0)}</span>
                              <span className="flex items-center gap-1 text-error font-medium"><XCircle className="h-3 w-3" />{subjectSummaries.reduce((s, c) => s + c.absent, 0)}</span>
                            </div>
                          )}
                    ...
                    <motion.div initial={{ opacity: 0, height: 0 }} animate={{ opacity: 1, height: "auto" }} exit={{ opacity: 0, height: 0 }} transition={{ duration: 0.2 }} className="overflow-hidden">
                      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4 pl-2 border-l-2 border-indigo-200">
                        {subjectSummaries.map((cls, index) => (
                          <motion.div key={cls.name} initial={{ opacity: 0, y: 8 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: index * 0.04 }}>
                            <Card className="cursor-pointer hover:shadow-card-hover hover:border-indigo-400/40 transition-all group" onClick={() => openClassAttendance(cls.name)}>
                              <CardContent className="p-5">
                                <div className="flex items-center justify-between mb-4">
                                  <div className="flex items-center gap-2">
                                    <div className="w-9 h-9 rounded-[10px] bg-indigo-50 flex items-center justify-center"><BookOpen className="h-4 w-4 text-indigo-600" /></div>
                                    <div>
                                      <h3 className="font-semibold text-text-primary text-sm leading-tight">{cls.displayName}</h3>
            ...
            {o2oGroups.length > 0 && (
              <div className="space-y-3">
                <button type="button" onClick={() => setO2oExpanded((v) => !v)} className="w-full text-left">
                  <Card className="hover:shadow-card-hover hover:border-primary/20 transition-all">
                    <CardContent className="p-5">
                      <div className="flex items-center justify-between">
                        <div className="flex items-center gap-3">
                          <div className="w-9 h-9 rounded-[10px] bg-amber-50 flex items-center justify-center"><Users className="h-4 w-4 text-amber-600" /></div>
                          <div>
                        ...
                        <div className="flex items-center gap-3">
                          {o2oSummaries.some((s) => s.total > 0) && (
                            <div className="flex items-center gap-2 text-xs">
                              <span className="flex items-center gap-1 text-success font-medium"><CheckCircle className="h-3 w-3" />{o2oSummaries.reduce((s, c) => s + c.present, 0)}</span>
                              <span className="flex items-center gap-1 text-error font-medium"><XCircle className="h-3 w-3" />{o2oSummaries.reduce((s, c) => s + c.absent, 0)}</span>
                            </div>
                          )}
                    ...
                    <motion.div initial={{ opacity: 0, height: 0 }} animate={{ opacity: 1, height: "auto" }} exit={{ opacity: 0, height: 0 }} transition={{ duration: 0.2 }} className="overflow-hidden">
                      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4 pl-2 border-l-2 border-amber-200">
                        {o2oSummaries.map((cls, index) => (
                          <motion.div key={cls.name} initial={{ opacity: 0, y: 8 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: index * 0.04 }}>
                            <Card className="cursor-pointer hover:shadow-card-hover hover:border-amber-400/40 transition-all group" onClick={() => openClassAttendance(cls.name)}>
                              <CardContent className="p-5">
                                <div className="flex items-center justify-between mb-4">
                                  <div className="flex items-center gap-2">
                                    <div className="w-9 h-9 rounded-[10px] bg-amber-50 flex items-center justify-center"><GraduationCap className="h-4 w-4 text-amber-600" /></div>
                                    <div>
                                      <h3 className="font-semibold text-text-primary text-sm leading-tight">{cls.displayName}</h3>
                                      <p className="text-[10px] text-text-tertiary font-mono truncate max-w-[140px]">{cls.name}</p>
                                    </div>
          ...
          <motion.div key="batch-detail" initial={{ opacity: 0, x: 20 }} animate={{ opacity: 1, x: 0 }} exit={{ opacity: 0, x: 20 }} className="space-y-6">
            <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
              <div className="flex items-center gap-3">
            ...
            {batchSchedulesLoading && <GifLoader />}
            ...
            {!batchSchedulesLoading && batchSchedules.length === 0 && (
              <div className="text-center py-10 text-text-secondary text-sm">
                ...
                <p className="text-xs mt-1 text-text-tertiary">No classes scheduled for this batch on this date.</p>
            ...
            {!batchSchedulesLoading && batchSchedules.length > 0 && (
              <div className="space-y-4">
                {batchSchedules.map((schedule) => {
                  ...
                  return (
                    <motion.div key={schedule.name} initial={{ opacity: 0, y: 8 }} animate={{ opacity: 1, y: 0 }}>
                      <Card
                        ...
                        onClick={!isExpanded ? () => handleSessionClick(schedule) : undefined}
                      >
                        <CardContent className="p-4">
                          <div className="flex items-start justify-between gap-3">
                            <div className="flex items-start gap-3 flex-1 min-w-0">
                              <div className={`w-10 h-10 rounded-[10px] flex items-center justify-center shrink-0 ${state === "completed" ? "bg-success-light" : "bg-brand-wash"}`}>
                                {state === "completed" ? <CheckCircle className="h-4 w-4 text-success" /> : <BookOpen className="h-4 w-4 text-primary" />}
                              </div>
                              <div className="min-w-0 flex-1">
                                ...
                                {schedule.custom_topic && <p className="text-xs text-text-secondary truncate">{schedule.course}</p>}
                                <div className="flex items-center gap-3 mt-1.5">
                                  <div className="flex items-center gap-1">
                                    <GraduationCap className="h-3 w-3 text-text-tertiary" />
                                    <span className="text-xs text-text-secondary">{schedule.student_group}</span>
                                  </div>
                                  {schedule.instructor_name && <span className="text-xs text-text-tertiary">{schedule.instructor_name}</span>}
                                </div>
                              </div>
                            </div>
                            <div className="flex items-center gap-2 shrink-0">
                              {state === "completed" && <Badge variant="success" className="text-[10px]">Saved</Badge>}
                              {state === "ready" && !isExpanded && <Badge variant="info" className="text-[10px]">Ready</Badge>}
                            ...
                            <motion.div initial={{ height: 0, opacity: 0 }} animate={{ height: "auto", opacity: 1 }} exit={{ height: 0, opacity: 0 }} transition={{ duration: 0.2 }} className="overflow-hidden">
                              <div className="border-t border-border px-4 pb-4 pt-3">
                                {sessionLoading ? (
                                  ...
                                  <p className="text-center py-8 text-sm text-text-secondary">No students found in this batch.</p>
                                ) : (
                                  <>
                                    ...
                                    <div className="flex items-center justify-between mb-3">
                                      <p className="text-xs text-text-tertiary"><Users className="h-3 w-3 inline mr-1" />{students.length} students</p>
                                      <div className="flex gap-2">
                                        <Button variant="outline" size="sm" onClick={(e) => { e.stopPropagation(); markAllPresent(); }}>
                                          <CheckCircle className="h-3.5 w-3.5" /> All Present
                                        </Button>
                                        <Button variant="primary" size="sm" onClick={(e) => { e.stopPropagation(); saveSessionAttendance(schedule); }} disabled={!!isSaving}>
                                      ...
                                      {students.map((student, index) => {
                                        ...
                                        return (
                                          <motion.button
                                            ...
                                            whileTap={{ scale: 0.97 }}
                                            onClick={(e) => { e.stopPropagation(); toggleStatus(student.student); }}
                                          ...
                                          >
                                            ...
                                            <Badge variant={status === "Present" ? "success" : status === "Absent" ? "error" : "warning"} className="text-[10px]">
                                              {status}
                                            </Badge>
                                          </motion.button>
                                        );
