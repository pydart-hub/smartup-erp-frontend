...
function SubjectAdmitPageContent() {
  ...
  const canUseManualDiscount = canApplyManualDiscount(selectedBranch);
  const canUseVennalaDiscount = canApplyVennalaPlusTwoDiscount(selectedBranch, selectedProgram);
                    ...
                    <div className="space-y-2">
                      ...
                      <div className="bg-blue-50 rounded-[12px] border border-blue-200 p-4 space-y-3">
                        <p className="text-xs text-blue-800">
                          Apply a percentage discount on total fees for Plus Two old students at Vennala. The discount is deducted from the last invoice first, then earlier invoices if needed.
                        </p>
                            ...
                            {(() => {
                              ...
                              return (
                                <div className="flex items-center gap-2 flex-wrap">
                                  <span className="text-xs text-blue-700">{pct}% of ₹{baseTotal.toLocaleString("en-IN")}</span>
                                  <span className="text-xs font-bold text-white bg-green-600 rounded-full px-2 py-0.5">−₹{discountAmt.toLocaleString("en-IN")} discount</span>
                                  <span className="text-xs text-blue-600">deducted from the last invoice first</span>
                                </div>
                              );
                            })()}
                          </div>
