...
export default function SalesOverdueBatchPage() {
  ...
  const { data: batches, isLoading, isError } = useQuery({
    queryKey: ["sales-dues-batches", branch, classId, asOf],
  ...
  return (
    ...
    >
      ...
      <motion.div variants={itemVariants} className="flex items-center gap-3">
        <Link
          href={`/dashboard/sales-user/fees/overdue/${encodeURIComponent(branch)}${childQs}`}
          className="text-text-tertiary hover:text-primary transition-colors"
        >
          <ArrowLeft className="h-5 w-5" />
        </Link>
        <div>
          <h1 className="text-2xl font-bold text-text-primary">{displayClass} — Overdue</h1>
          <p className="text-sm text-text-secondary mt-0.5">
            Batch-wise breakdown at {shortBranch}
          </p>
        </div>
      </motion.div>
      ...
      {isLoading ? (
        ...
        <div className="flex flex-col items-center justify-center h-48 gap-3">
          ...
          <p className="text-sm text-success font-medium">No overdue fees for this class!</p>
        </div>
      ) : (
        <motion.div initial="hidden" animate="visible" variants={containerVariants} className="space-y-2">
          {batches.map((batch) => (
            <motion.div key={batch.batch_id} variants={itemVariants}>
              <Link
                href={`/dashboard/sales-user/fees/overdue/${encodeURIComponent(branch)}/${encodeURIComponent(classId)}/${encodeURIComponent(batch.batch_id)}${childQs}`}
              >
