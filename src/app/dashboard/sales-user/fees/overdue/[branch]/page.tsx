...
export default function SalesOverdueClassPage() {
  ...
  return (
    ...
    >
      ...
      <motion.div variants={itemVariants} className="flex items-center gap-3">
        <Link href={`/dashboard/sales-user/fees/overdue${childQs}`} className="text-text-tertiary hover:text-primary transition-colors">
          <ArrowLeft className="h-5 w-5" />
        </Link>
        <div>
          <h1 className="text-2xl font-bold text-text-primary">{shortName} — Overdue</h1>
          <p className="text-sm text-text-secondary mt-0.5">Class-wise overdue breakdown</p>
        </div>
      </motion.div>
      ...
      {isLoading ? (
        ...
        <div className="flex flex-col items-center justify-center h-48 gap-3">
          ...
          <p className="text-sm text-success font-medium">No overdue fees for this branch!</p>
        </div>
      ) : (
          ...
          {classes.map((cls) => {
            ...
            return (
              <motion.div key={cls.item_code} variants={itemVariants}>
                <Link
                  href={`/dashboard/sales-user/fees/overdue/${encodeURIComponent(branch)}/${encodeURIComponent(cls.item_code)}${childQs}`}
                >
                  <div className="flex items-center gap-3 p-4 rounded-[10px] border border-border-light hover:border-orange-300/50 hover:shadow-sm transition-all cursor-pointer bg-surface">
                    ...
                    <div className="flex-1 min-w-0">
                      ...
                      <p className="text-xs text-text-tertiary mt-0.5">
                        {cls.student_count} student{cls.student_count !== 1 ? "s" : ""} · {cls.invoice_count} invoice{cls.invoice_count !== 1 ? "s" : ""}
                      </p>
                    </div>
