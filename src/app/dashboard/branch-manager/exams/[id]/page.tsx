...
import { deleteExam, getAssessmentPlan, getExamResults, saveMarks } from "@/lib/api/assessment";
...
export default function ExamMarkEntryPage() {
  ...
  const deleteMutation = useMutation({
    ...
    onSuccess: () => {
      ...
      queryClient.invalidateQueries({ queryKey: ["assessment-plans"] });
      queryClient.removeQueries({ queryKey: ["assessment-plan", examId] });
      queryClient.removeQueries({ queryKey: ["exam-results", examId] });
      router.push("/dashboard/branch-manager/exams");
    },
    onError: (error: Error) => {
  ...
  function handleDelete() {
    if (existingResults?.data?.length) {
      toast.error("Cannot delete this exam because marks/results already exist.");
      return;
    }

    const confirmed = window.confirm(
      `Delete the scheduled exam \"${plan?.course || examId}\"? This cannot be undone.`
    );
    if (!confirmed) return;
      ...
      <div className="space-y-6">
        ...
        <Card>
          <CardContent className="p-12 text-center">
            ...
            <p className="text-sm text-text-secondary">The exam &quot;{examId}&quot; could not be loaded.</p>
          </CardContent>
        </Card>
      </div>
    );
    ...
    <motion.div variants={container} initial="hidden" animate="show" className="space-y-6">
      ...
      <motion.div variants={item} className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
        ...
        <div className="flex gap-2">
          <Button
            ...
            disabled={isDeleting || isSaving || hasExistingResults}
            title={hasExistingResults ? "Delete is disabled because marks/results already exist" : undefined}
          >
            {isDeleting ? <Loader2 className="h-4 w-4 animate-spin" /> : <Trash2 className="h-4 w-4" />}
            {isDeleting ? "Deleting..." : "Delete Exam"}
          </Button>
          <Button variant="primary" size="md" onClick={handleSave} disabled={isSaving || isDeleting || marks.length === 0}>
        ...
        <Card>
          <CardContent className="p-4">
            <div className="flex flex-wrap gap-x-6 gap-y-2 text-sm text-text-secondary">
              <span className="flex items-center gap-1.5"><Calendar className="h-4 w-4 text-text-tertiary" />{formatDate(plan.schedule_date)}</span>
              <span className="flex items-center gap-1.5"><Clock className="h-4 w-4 text-text-tertiary" />{formatTime12h(plan.from_time)} - {formatTime12h(plan.to_time)}</span>
              <span className="flex items-center gap-1.5"><Hash className="h-4 w-4 text-text-tertiary" />Max Score: {plan.maximum_assessment_score}</span>
              <span className="flex items-center gap-1.5"><Users className="h-4 w-4 text-text-tertiary" />{marks.length} students</span>
              {plan.examiner_name && <span>Examiner: {plan.examiner_name}</span>}
            </div>
            {hasExistingResults && (
              <p className="mt-3 text-xs text-text-tertiary">Delete is disabled because marks/results already exist for this exam.</p>
            )}
          </CardContent>
        </Card>
      </motion.div>
      ...
      {stats.filled > 0 && (
        <motion.div variants={item} className="grid grid-cols-2 sm:grid-cols-5 gap-3">
          <div className="bg-surface rounded-[12px] p-3 text-center border border-border-light"><p className="text-xl font-bold text-text-primary">{stats.filled}/{stats.total}</p><p className="text-xs text-text-secondary mt-0.5">Entered</p></div>
          <div className="bg-success-light rounded-[12px] p-3 text-center border border-success/10"><p className="text-xl font-bold text-success">{stats.passed}</p><p className="text-xs text-success mt-0.5">Passed</p></div>
          <div className="bg-error-light rounded-[12px] p-3 text-center border border-error/10"><p className="text-xl font-bold text-error">{stats.failed}</p><p className="text-xs text-error mt-0.5">Failed</p></div>
          <div className="bg-brand-wash rounded-[12px] p-3 text-center border border-primary/10"><p className="text-xl font-bold text-primary">{stats.avg}</p><p className="text-xs text-primary mt-0.5">Average</p></div>
          <div className="bg-surface rounded-[12px] p-3 text-center border border-border-light"><p className="text-xl font-bold text-text-primary">{stats.highest}</p><p className="text-xs text-text-secondary mt-0.5">Highest</p></div>
        </motion.div>
      )}
        ...
        <Card>
          <CardHeader>
            <div className="flex items-center justify-between">
              <CardTitle className="flex items-center gap-2"><FileText className="h-5 w-5 text-text-tertiary" />Mark Entry</CardTitle>
              {hasExistingResults && <Badge variant="warning">Editing existing marks</Badge>}
            </div>
          </CardHeader>
          <CardContent>
            {marks.length === 0 ? (
              <p className="text-sm text-text-secondary text-center py-8">No students found in this batch.</p>
            ) : (
                    ...
                    {marks.map((m, idx) => {
                      ...
                      return (
                        ...
                        >
                          ...
                          <td className="px-3 py-3 text-text-secondary text-xs">{m.student}</td>
