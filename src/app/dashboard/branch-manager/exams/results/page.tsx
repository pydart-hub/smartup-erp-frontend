...
export default function ExamResultsPage() {
  ...
  const { data: studentGroups = [] } = useQuery({
    queryKey: ["student-groups-for-results", defaultCompany],
    queryFn: async () => {
    ...
    >
      ...
      <motion.div variants={item}>
        ...
        <p className="text-sm text-text-secondary mt-0.5">
          View batch-wide results, rankings, and individual report cards
        </p>
      </motion.div>
            ...
            <div className="flex flex-col sm:flex-row gap-3">
              ...
              <div className="flex flex-col gap-1.5 flex-1">
                ...
                <select
                  ...
                  onChange={(e) => {
                    setAssessmentGroup(e.target.value);
                    setSelectedStudent(null);
                  }}
                  className="h-10 rounded-[10px] border border-border-input bg-surface px-3 text-sm"
                >
          ...
          <Card>
            <CardContent className="p-12 text-center">
              ...
              <h3 className="text-lg font-semibold text-text-primary mb-1">
                Select Batch & Exam Type
              </h3>
              <p className="text-sm text-text-secondary">
                Choose a batch and exam type above to view results.
              </p>
            </CardContent>
          </Card>
        </motion.div>
        ...
        <motion.div variants={item} className="grid grid-cols-2 sm:grid-cols-6 gap-3">
          ...
          <div className="bg-error-light rounded-[12px] p-3 text-center border border-error/10">
            <p className="text-xl font-bold text-error">
              {summary.total_students - summary.pass_count}
            </p>
              ...
              <div className="flex items-center justify-between gap-4">
                ...
                <div className="relative">
                  ...
                  <input
                    ...
                    className="h-9 pl-9 pr-3 rounded-[8px] border border-border-input bg-surface text-sm w-56"
                  />
                </div>
              </div>
            </CardHeader>
                  ...
                  <thead>
                    <tr className="border-b border-border-light">
                      ...
                      {results[0]?.subjects?.map((s) => (
                        <th
                          key={s.course}
                          className="text-center px-3 py-3 font-medium text-text-secondary"
                        >
                          {s.course}
                        </th>
                      ))}
                  ...
                  <tbody>
                    {filtered.map((r, idx) => (
                      ...
                      >
                        ...
                        {r.subjects?.map((s) => (
                          <td
                            ...
                            className={`text-center px-3 py-3 ${
                              !s.passed ? "text-error" : "text-text-primary"
                            }`}
                          >
                            {s.score}/{s.maximum_score}
                          </td>
                        ))}
                        <td className="text-center px-3 py-3 font-semibold text-text-primary">
                          {r.total_score}/{r.total_maximum}
                        </td>
                        <td className="text-center px-3 py-3 font-semibold text-primary">
                          {r.overall_percentage}%
                        </td>
                        <td className="text-center px-3 py-3">
                          <Badge variant="outline">{r.overall_grade}</Badge>
                        </td>
                        <td className="text-center px-3 py-3">
                          {r.passed ? (
                        ...
                        <td className="text-center px-3 py-3">
                          <button
                            onClick={() => setSelectedStudent(r.student)}
          ...
          <motion.div variants={item}>
            <Card>
              <CardContent className="p-12 text-center">
                ...
                <p className="text-sm text-text-secondary">
                  No marks have been entered for this batch and exam type.
                </p>
              </CardContent>
            </Card>
          </motion.div>
          ...
          <Card className="border-primary/20">
            ...
            <CardContent>
              {reportLoading ? (
                ...
                <div className="space-y-4 print:space-y-2" id="report-card">
                  {/* Student Info */}
                  <div className="bg-brand-wash rounded-[12px] p-4 border border-primary/10 print:bg-white print:border-black">
                    <div className="grid grid-cols-2 sm:grid-cols-4 gap-3 text-sm">
                      <div>
                        <p className="text-text-secondary text-xs">Student</p>
                        <p className="font-semibold text-text-primary">
                          {reportCard.data.student_name}
                        </p>
                      </div>
                      <div>
                        <p className="text-text-secondary text-xs">ID</p>
                        <p className="font-medium text-text-primary">
                          {reportCard.data.student}
                        </p>
                      </div>
                      <div>
                        <p className="text-text-secondary text-xs">Program</p>
                        <p className="font-medium text-text-primary">
                          {reportCard.data.program}
                        </p>
                      </div>
                      <div>
                        <p className="text-text-secondary text-xs">Rank</p>
                        <p className="font-bold text-primary text-lg">
                          #{reportCard.data.rank}
                        </p>
                      </div>
                    </div>
                  </div>
                  ...
                  <table className="w-full text-sm border border-border-light rounded-[10px] overflow-hidden">
                    ...
                    <tfoot>
                      <tr className="bg-app-bg border-t border-border-light font-semibold">
                        ...
                        <td className="text-center px-4 py-2.5">
                          {reportCard.data.total_score}
                        </td>
                        <td className="text-center px-4 py-2.5 text-text-secondary">
                          {reportCard.data.total_maximum}
                        </td>
                        <td className="text-center px-4 py-2.5 text-primary">
                          {reportCard.data.overall_percentage}%
                        </td>
                        <td className="text-center px-4 py-2.5">
                          <Badge variant="outline">
                            {reportCard.data.overall_grade}
                          </Badge>
                        </td>
                        <td className="text-center px-4 py-2.5">
                          {reportCard.data.passed ? (
                            <Badge variant="success">PASS</Badge>
                          ) : (
                            <Badge variant="error">FAIL</Badge>
                          )}
