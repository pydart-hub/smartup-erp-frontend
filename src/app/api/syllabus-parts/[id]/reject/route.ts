...
/**
 * PATCH /api/syllabus-parts/[id]/reject
 * BM rejects a part → status="Rejected"
...
export async function PATCH(
  ...
  try {
    ...
    // Only reject from "Pending Approval"
    if (record.status !== "Pending Approval") {
      return NextResponse.json(
        { error: `Cannot reject from status: ${record.status}` },
        { status: 400 },
      );
    }
    ...
    if (!res.ok) {
      ...
      console.error("[syllabus-parts/reject] error:", res.status, errText);
    ...
    console.error("[syllabus-parts/reject] error:", error);
