...
export async function GET(
  ...
  try {
    ...
    const data = await frappeLevelExamStore.getAttemptPayload(attemptId, studentId);
    if (!data) {
      return NextResponse.json({ error: "Attempt not found" }, { status: 404 });
    }
    if (data.remaining_seconds <= 0) {
      const result = await frappeLevelExamStore.submitAttempt(attemptId, studentId, true);
      return NextResponse.json({ error: "Attempt timed out", result }, { status: 409 });
    }
    return NextResponse.json({ data });
  } catch (error: unknown) {
    const err = error as { message?: string };
    const status = err.message === "Not authenticated" ? 401 : 500;
    return NextResponse.json({ error: err.message || "Failed to fetch attempt" }, { status });
  }
}
