...
export async function POST(req: NextRequest) {
  try {
    ...
    const isDummy = email.endsWith("@dummy.com");

    // Send email (skip for auto-generated @dummy.com addresses)
    let emailSent = false;
    if (isDummy) {
      console.log(`[send-student-welcome] Skipping email for dummy address: ${email}`);
    } else {
      const emailBody = `
