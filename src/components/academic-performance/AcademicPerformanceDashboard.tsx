...
export function AcademicPerformanceDashboard({
  basePath = "/dashboard/director/academic-performance",
  roleTitle = "Director",
}: AcademicPerformanceDashboardProps) {
  ...
  const { data: branchesData, isLoading: loadingBranches } = useQuery({
    queryKey: ["academic-perf-branches", basePath],
  ...
  const { data, isLoading: loadingData } = useQuery<ClassPerformanceResponse>({
    queryKey: ["branch-perf", selectedBranch],
        ...
        <div className="flex items-center gap-3">
          ...
          <div>
            ...
            <p className="text-xs sm:text-sm text-text-tertiary">
              {roleTitle} Overview: Instant branch comparison with unified multi-class examination analytics.
            </p>
          </div>
        </div>
                  ...
                  <div className="flex items-center gap-3 text-[11px] text-text-tertiary">
                    {selectedClass !== "all" && (
                      <button
                        ...
                        onClick={() =>
                          router.push(
                            `${basePath}/class?branch=${encodeURIComponent(
                              selectedBranch,
