...
export interface AcademicPerformanceClassViewProps {
  basePath?: string;
  rolePrefix?: string;
}

export function AcademicPerformanceClassView({
  basePath = "/dashboard/director/academic-performance",
  rolePrefix = "/dashboard/director",
}: AcademicPerformanceClassViewProps) {
  ...
  const { data, isLoading } = useQuery<ClassPerformanceResponse>({
    queryKey: ["class-detail", basePath, branch, program, selectedBatch],
    queryFn: () =>
                  ...
                  <div className="w-full overflow-x-auto no-scrollbar">
                    <svg
                      ...
                      onDoubleClick={() => {
                        if (selectedBatch) {
                          router.push(
                            `${basePath}/batch?branch=${encodeURIComponent(
                              branch,
                    ...
                    >
                      <defs>
                        <linearGradient id={`classGrad-${program.replace(/\s+/g, "_")}`} x1="0" y1="0" x2="0" y2="1">
                      ...
                      {svgPoints.length > 1 && !selectedBatch && (
                        <path d={areaPath} fill={`url(#classGrad-${program.replace(/\s+/g, "_")})`} />
                      )}
