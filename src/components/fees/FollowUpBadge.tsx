 ...
 * Displays: status label · call date · optional paid amount + remarks below if available
 */
export function FollowUpBadge({ log }: FollowUpBadgeProps) {
  ...
  return (
    <div className="flex flex-col gap-1">
      <div
        className={`inline-flex items-center gap-1.5 rounded-full px-2.5 py-1 text-[10px] font-medium border ${colors.bg} ${colors.text} ${colors.border} w-fit`}
      >
        {/* Colored dot */}
      ...
      {log.remarks && (
        <p className="text-xs text-text-secondary line-clamp-2">{log.remarks}</p>
      )}
    </div>
  );
}
