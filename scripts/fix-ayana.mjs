...
async function main() {
  const studentId = "STU-SU FKO-26-096"; // AYANA RAJEEV
  console.log(`=== RECONCILING STUDENT: AYANA RAJEEV (${studentId}) ===`);

  // Step 1: Cancel Payment Entry ACC-PAY-2026-05551 (which links to invoice 6)
  const peName = "ACC-PAY-2026-05551";
  ...
  // Step 2: Cancel active Sales Invoices for due dates 2026-08-15, 2026-09-15, 2026-10-15, 2026-11-15
  const dueDates = ["2026-08-15", "2026-09-15", "2026-10-15", "2026-11-15"];
  ...
  const templateDoc = await frappeGet(`resource/Sales Invoice/ACC-SINV-2026-05417`);
  const firstItem = templateDoc.items[0];

  // Step 3: Re-create Invoices 6..9 with matching payment_schedule
  const invConfigs = [
    { dueDate: "2026-08-15", rate: 270 },
    { dueDate: "2026-09-15", rate: 2500 },
    { dueDate: "2026-10-15", rate: 2500 },
    { dueDate: "2026-11-15", rate: 1130 },
  ];
  ...
  // Step 4: Re-create Payment Entry ACC-PAY-2026-05551 allocating to new Invoice 6 (2026-08-15)
  const newInv6 = createdInvs.find((i) => i.due_date === "2026-08-15");
  const newPePayload = {
    doctype: "Payment Entry",
    payment_type: peDoc.payment_type,
    posting_date: peDoc.posting_date, // 2026-06-06
    ...
    paid_amount: peDoc.paid_amount, // 270
    received_amount: peDoc.received_amount, // 270
  ...
  console.log(`Re-created Payment Entry => ${submittedPE.name} | Amount: ₹${submittedPE.paid_amount} | Date: ${submittedPE.posting_date} | Mode: ${submittedPE.mode_of_payment} | Linked to: ${newInv6.name}`);

  // Step 5: Verification
  console.log("\n=== VERIFICATION FOR AYANA RAJEEV ===");
