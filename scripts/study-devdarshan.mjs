...
async function run() {
  try {
    console.log('=== STUDYING DEVDARSHAN (SRR 303) ===\n');

    // 1. Student Record
    let studentRes = await fetch(`${baseUrl}/api/resource/Student?filters=${encodeURIComponent(JSON.stringify([["first_name","like","%Devdarshan%"]]))}&fields=${encodeURIComponent(JSON.stringify(["*"]))}`, { headers });
    let studentData = await studentRes.json();
    
    if (!studentData.data || studentData.data.length === 0) {
      studentRes = await fetch(`${baseUrl}/api/resource/Student?filters=${encodeURIComponent(JSON.stringify([["custom_srr_id","=","303"]]))}&fields=${encodeURIComponent(JSON.stringify(["*"]))}`, { headers });
    ...
    let studentName = studentData.data?.[0]?.customer || 'Devdarshan';
    let studentId = studentData.data?.[0]?.name;

    // 2. Sales Orders
    let soRes = await fetch(`${baseUrl}/api/resource/Sales Order?filters=${encodeURIComponent(JSON.stringify([["customer_name","like","%Devdarshan%"]]))}&fields=${encodeURIComponent(JSON.stringify(["*"]))}`, { headers });
    ...
    if (soData.data) {
      for (let so of soData.data) {
        ...
        console.log(`\n--- SO ${so.name} ITEMS & PAYMENT SCHEDULE ---`);
    ...
    // 3. Sales Invoices
    let sinvRes = await fetch(`${baseUrl}/api/resource/Sales Invoice?filters=${encodeURIComponent(JSON.stringify([["customer_name","like","%Devdarshan%"]]))}&fields=${encodeURIComponent(JSON.stringify(["*"]))}`, { headers });
    ...
    if (sinvData.data) {
      for (let inv of sinvData.data) {
        ...
        console.log('Items:', singleInv.data.items.map(i => ({ code: i.item_code, name: i.item_name, qty: i.qty, rate: i.rate, amount: i.amount, sales_order: i.sales_order })));
      }
    }

    // 4. Payment Entries
    let peRes = await fetch(`${baseUrl}/api/resource/Payment Entry?filters=${encodeURIComponent(JSON.stringify([["party_name","like","%Devdarshan%"]]))}&fields=${encodeURIComponent(JSON.stringify(["*"]))}`, { headers });
