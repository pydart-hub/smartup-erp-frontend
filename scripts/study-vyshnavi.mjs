...
async function run() {
  try {
    console.log('=== STUDYING VYSHNAVI JAYARAJ (SRR 306) ===\n');

    // 1. Student Record
    let studentRes = await fetch(`${baseUrl}/api/resource/Student?filters=${encodeURIComponent(JSON.stringify([["first_name","like","%Vyshnavi%"]]))}&fields=${encodeURIComponent(JSON.stringify(["*"]))}`, { headers });
    let studentData = await studentRes.json();
    
    if (!studentData.data || studentData.data.length === 0) {
      studentRes = await fetch(`${baseUrl}/api/resource/Student?filters=${encodeURIComponent(JSON.stringify([["custom_srr_id","=","306"]]))}&fields=${encodeURIComponent(JSON.stringify(["*"]))}`, { headers });
    ...
    // 2. Sales Orders
    let soRes = await fetch(`${baseUrl}/api/resource/Sales Order?filters=${encodeURIComponent(JSON.stringify([["customer_name","like","%Vyshnavi%"]]))}&fields=${encodeURIComponent(JSON.stringify(["*"]))}`, { headers });
    ...
    // 3. Sales Invoices
    let sinvRes = await fetch(`${baseUrl}/api/resource/Sales Invoice?filters=${encodeURIComponent(JSON.stringify([["customer_name","like","%Vyshnavi%"]]))}&fields=${encodeURIComponent(JSON.stringify(["*"]))}`, { headers });
    ...
    // 4. Payment Entries
    let peRes = await fetch(`${baseUrl}/api/resource/Payment Entry?filters=${encodeURIComponent(JSON.stringify([["party_name","like","%Vyshnavi%"]]))}&fields=${encodeURIComponent(JSON.stringify(["*"]))}`, { headers });
