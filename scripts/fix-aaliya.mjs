...
async function main() {
  const studentId = "STU-SU FKO-26-069"; // AALIYA MIRZA
  console.log(`=== RECONCILING STUDENT: AALIYA MIRZA (${studentId}) ===`);

  // Step 1: Cancel Payment Entry ACC-PAY-2026-06342 (which links to invoice 3)
  const peName = "ACC-PAY-2026-06342";
  ...
  const templateDoc = await frappeGet(`resource/Sales Invoice/ACC-SINV-2026-04278`);
  const firstItem = templateDoc.items[0];

  // Step 3: Re-create Invoices 3..8 with matching payment_schedule
  const invConfigs = [
    { dueDate: "2026-06-15", rate: 2900 },
    { dueDate: "2026-07-15", rate: 1900 },
    { dueDate: "2026-08-15", rate: 1900 },
    { dueDate: "2026-09-15", rate: 1900 },
    { dueDate: "2026-10-15", rate: 1900 },
    { dueDate: "2026-11-15", rate: 1900 },
  ];
  ...
  // Step 4: Re-create Payment Entry ACC-PAY-2026-06342 allocating to new Invoice 3 (2026-06-15)
  const newInv3 = createdInvs.find((i) => i.due_date === "2026-06-15");
  const newPePayload = {
    doctype: "Payment Entry",
    payment_type: peDoc.payment_type,
    posting_date: peDoc.posting_date, // 2026-07-02
    ...
    paid_amount: peDoc.paid_amount, // 2900
    received_amount: peDoc.received_amount, // 2900
