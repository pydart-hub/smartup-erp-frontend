...
async function run() {
  try {
    console.log('=== EXECUTION START: VYSHNAVI JAYARAJ (SRR 306) ===');

    // 1. Cancel old Demo Sales Invoice (ACC-SINV-2026-10686)
    console.log('\nStep 1: Cancelling Demo Sales Invoice ACC-SINV-2026-10686...');
    let invCancelRes = await fetch(`${baseUrl}/api/resource/Sales Invoice/ACC-SINV-2026-10686`, {
      method: 'PUT',
      headers,
    ...
    if (invCancelRes.ok) {
      console.log('Successfully cancelled Demo Sales Invoice ACC-SINV-2026-10686');
    } else {
      console.error('Error cancelling Sales Invoice:', invCancelData);
    }

    // 2. Cancel old Demo Sales Order (SAL-ORD-2026-01478)
    console.log('\nStep 2: Cancelling Demo Sales Order SAL-ORD-2026-01478...');
    let soCancelRes = await fetch(`${baseUrl}/api/resource/Sales Order/SAL-ORD-2026-01478`, {
      method: 'PUT',
      headers,
      body: JSON.stringify({ docstatus: 2 })
    });
    ...
    if (soCancelRes.ok) {
      console.log('Successfully cancelled Demo Sales Order SAL-ORD-2026-01478');
    } else {
      console.error('Error cancelling Sales Order:', soCancelData);
    }

    // 3. Fetch details of main Sales Order SAL-ORD-2026-01762
    console.log('\nStep 3: Fetching main Sales Order SAL-ORD-2026-01762 details...');
    let soMainRes = await fetch(`${baseUrl}/api/resource/Sales Order/SAL-ORD-2026-01762`, { headers });
    let soMainData = await soMainRes.json();
    let mainSO = soMainData.data;

    if (!mainSO) {
      throw new Error('Sales Order SAL-ORD-2026-01762 not found');
    }
    ...
    const installments = [
      { num: 1, posting_date: '2026-07-04', due_date: '2026-07-04' },
