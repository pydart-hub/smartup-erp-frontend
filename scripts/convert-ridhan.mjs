...
async function main() {
  console.log("🚀 EXECUTING CONVERSION FOR RIDHAN SADATH");
  console.log("=".repeat(70));
  
  const STUDENT_ID = "STU-SU ERV-26-212";
  const CUSTOMER = "RIDHAN SADATH";
  const COMPANY = "Smart Up Eraveli";
  const ITEM_CODE = "9th State Tuition Fee";
  const ACADEMIC_YEAR = "2026-2027";
  
  const OLD_SO = "SAL-ORD-2026-01188";
  const OLD_PE = "ACC-PAY-2026-05362";
  const OLD_INVOICES = [
    "ACC-SINV-2026-08766",
    "ACC-SINV-2026-08767",
    "ACC-SINV-2026-08768",
  ...
  const pePaidAmount = peDetail.paid_amount;   // 2000
  const peMode = peDetail.mode_of_payment;     // Cash
  const peRefNo = peDetail.reference_no;       // CASH-1780406648462
  ...
  // Step 5: Re-create & Submit Payment Entry (2000 total: 1700 to Inst 1, 300 to Inst 2)
  ...
  const pePayload = {
    ...
    reference_no: peRefNo,       // CASH-1780406648462
    reference_date: pePostingDate,
    paid_amount: pePaidAmount,   // 2000
  ...
  console.log(`  ✓ Created & Submitted PE (${createdPE.name}): ₹2,000 (₹1,700 to Inst 1, ₹300 to Inst 2)`);
  ...
  console.log("\n" + "=".repeat(70));
  console.log("🎉 CONVERSION COMPLETED SUCCESSFULLY FOR RIDHAN SADATH!");
  console.log("=".repeat(70));
  console.log(`Summary:`);
  console.log(`  Sales Order: ${createdSO.name} (Total: ₹${TOTAL_AMOUNT})`);
  console.log(`  Inst 1 Invoice: ${newInvoices[0].name} (₹1,700) → Status: PAID`);
  console.log(`  Inst 2 Invoice: ${newInvoices[1].name} (₹1,700) → Status: PARTIALLY PAID (₹1,400 outstanding)`);
  console.log(`  PE: ${createdPE.name} (₹2,000 on ${pePostingDate})`);
}
