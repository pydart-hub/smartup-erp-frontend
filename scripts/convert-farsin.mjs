...
async function main() {
  console.log("🚀 EXECUTING CONVERSION FOR FARSIN MOHAMMED VA");
  console.log("=".repeat(70));
  
  const STUDENT_ID = "STU-SU ERV-26-216";
  const CUSTOMER = "FARSIN MOHAMMED VA";
  const COMPANY = "Smart Up Eraveli";
  const ITEM_CODE = "8th State Tuition Fee";
  const ACADEMIC_YEAR = "2026-2027";
  
  const OLD_SO = "SAL-ORD-2026-01222";
  const OLD_PE = "ACC-PAY-2026-05439";
  const OLD_INVOICES = [
    "ACC-SINV-2026-09001",
    "ACC-SINV-2026-09002",
    "ACC-SINV-2026-09003",
  ...
  const TARGET_SCHEDULE = [
    { label: "Inst 1", amount: 1450, due_date: "2026-06-03" },
    { label: "Inst 2", amount: 1450, due_date: "2026-07-03" },
    { label: "Inst 3", amount: 1450, due_date: "2026-08-03" },
    { label: "Inst 4", amount: 1450, due_date: "2026-09-03" },
    { label: "Inst 5", amount: 1450, due_date: "2026-10-03" },
    { label: "Inst 6", amount: 1450, due_date: "2026-11-03" },
    { label: "Inst 7", amount: 1450, due_date: "2026-12-03" },
    { label: "Inst 8", amount: 1600, due_date: "2027-01-03" },
  ];
  ...
  const pePaidAmount = peDetail.paid_amount;   // 1800
  const peMode = peDetail.mode_of_payment;     // Cash
  const peRefNo = peDetail.reference_no;       // CASH-1780490955442
  ...
  // Step 5: Re-create & Submit Payment Entry (1800 total: 1450 to Inst 1, 350 to Inst 2)
  ...
  const pePayload = {
    ...
    reference_no: peRefNo,       // CASH-1780490955442
    reference_date: pePostingDate,
    paid_amount: pePaidAmount,   // 1800
  ...
  console.log(`  ✓ Created & Submitted PE (${createdPE.name}): ₹1,800 (₹1,450 to Inst 1, ₹350 to Inst 2)`);
  ...
  console.log("\n" + "=".repeat(70));
  console.log("🎉 CONVERSION COMPLETED SUCCESSFULLY FOR FARSIN MOHAMMED VA!");
  console.log("=".repeat(70));
  console.log(`Summary:`);
  console.log(`  Sales Order: ${createdSO.name} (Total: ₹${TOTAL_AMOUNT})`);
  console.log(`  Inst 1 Invoice: ${newInvoices[0].name} (₹1,450) → Status: PAID`);
  console.log(`  Inst 2 Invoice: ${newInvoices[1].name} (₹1,450) → Status: PARTIALLY PAID (₹1,100 outstanding)`);
  console.log(`  PE: ${createdPE.name} (₹1,800 on ${pePostingDate})`);
}
